...
public class PythonScript extends AbstractScriptTransformer {
	...
	private DataMessageComponent getPythonRpcMessage(final List<DataMessageComponent> cache) throws Exception {
		...
		try {
			...
			// Cut off all stack traces above and including "execfile(scriptPath, globals(), locals()) # need to place locals from script in globals"
			// since they are generated by DAWN python code (DAWNSCI-736)
			String stringToMatch = "execfile(scriptPath, globals(), locals()) # need to place locals from script in globals";
